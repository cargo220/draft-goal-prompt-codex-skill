...
description: "Draft concise, ready-to-paste Codex /goal prompts as a goal prompt harness with Mission, Current Goal, Goal Review, and full next-goal handoff framing. Use when a user asks for a goal prompt, next-session prompt, objective prompt, Mission/Current Goal framing, or phrases such as \"goal 프롬프트 만들어줘\", \"다음 세션 goal 써줘\", \"목표 프롬프트 작성해줘\", or when they want a vague task turned into an audit-friendly Codex goal."
---
...
Turn a user's rough next-session intent into a ready-to-paste Codex `/goal` prompt.
...
When a `Mission` remains unfinished after the `Current Goal`, require a next-goal handoff: the current session should propose concrete next `Current Goal` candidates, but must not execute them unless the user explicitly approves.
...
1. Decide whether the user needs a short single-goal prompt or a long-project framing. Use `Mission` only when a durable direction changes how the current goal should be scoped.
...
5. Never fabricate paths, tool names, repo state, or previous decisions. Use a TODO placeholder or ask the user when exact context is needed.
...
- Match the user's language for user-facing explanations and the final ready-to-paste `/goal` prompt body. If the user asks in Korean, write the goal in Korean. Keep section labels, code, commands, file names, local paths, and official terms in English when clearer unless the user asks to translate them too.
...
- For wiki or knowledge-base work, tell Codex to inspect the project's router files first, such as `AGENTS.md`, `README.md`, index files, and relevant task guides when they exist.
- For repository work, tell Codex to read the nearest project instructions such as `AGENTS.md` before changing files.
- For robotics, Physical AI, hardware, simulator, or runtime-heavy work, default to small hands-on steps with concrete artifacts and verification. Keep hardware actuation, large downloads, GPU-heavy runtimes, and large simulator stacks fenced off unless the user explicitly changes that boundary.
...
Stop if:
- <mechanically detectable conditions requiring user confirmation>

Handoff:
<optional; how a later Codex session should resume if interrupted, and for long Missions how the user should choose/approve the next Current Goal>
```
...
```text
Goal Review:
- What this goal will do: <short summary in the user's language>
- What this goal will not do: <clear exclusions>
- Completion check: <how the user/Codex can tell it is done>
...
- If bulky context makes the answer too long, use an inline `Detailed Handoff` block or ask the user before creating/updating a file.

## Section Guidance

- `Mission`: use for large efforts such as knowledge-base projects, robotics work, product migrations, or long learning programs. It supplies direction but must not be the finish line.
- `Current Goal`: phrase as a target state, not just activity. Prefer "produce/update/verify X" over "work on X".
- `First context to inspect`: list exact local paths when known. If a path is unknown, write `TODO: user to provide path` instead of inventing it.
...
Use this rule only when the user asks for unattended progress or when a long-running goal clearly benefits from it:

- If the only blocker is a simple "continue?" confirmation and the next step is already inside `Scope` and `Constraints`, wait 5 minutes for the user. If there is no reply, treat that as permission to continue with the safest in-scope next step.
...
- If the goal is already marked `blocked`, a later session still needs the user's resume/continue instruction or an explicit new `/goal`; the prompt cannot change that state by itself.
...
- Add a `Handoff` rule: "A later session should use only the user-approved candidate as the next Current Goal."
...
- Prefer inline `Detailed Handoff` for bulky context. Ask the user before creating a separate handoff file.
- Do not let Codex automatically continue into the recommended next goal unless the user explicitly asks for that behavior.
...
- Long-running `Mission` prompts include `Next Goal candidates`, `Recommended Next Goal`, and a complete `Ready-to-paste next /goal` requirement instead of leaving the user to invent the next step from scratch.
...
### Knowledge Wiki Mission + Current Goal

```text
Mission:
사용자의 Knowledge Wiki를 프로젝트 적용 지식이 재사용 가능한 하나의 큰 위키가 되도록 확장한다. 이 Mission은 방향이며 완료 조건이 아니다.

Current Goal:
Knowledge Wiki에서 다음으로 진행할 하나의 작은 확장 목표를 정하고, 그 목표를 실행 가능한 계획으로 정리한다. 이번 세션에서는 파일 이동이나 대량 작성까지 하지 말고, 첫 번째로 만들거나 갱신할 노트 묶음과 검증 방법을 확정한다.

First context to inspect:
- TODO: path to the wiki or knowledge-base `AGENTS.md`
- TODO: path to the wiki `README.md`
- TODO: path to the main index file
- TODO: path to relevant topic index files

Scope:
- In scope: Knowledge Wiki의 다음 한 조각을 고르는 계획, 후보 노트 경로, 링크 전략, 검증 방법.
- Out of scope: 실제 대량 작성, 폴더 이동, 기존 노트 삭제, 스킬 설치.

Constraints:
- Korean-facing summary를 기본으로 쓴다.
- 현재 로컬 파일을 먼저 읽고, 없는 경로를 추측하지 않는다.
- 대용량 다운로드, 외부 서비스 설정, 런타임-heavy 실행은 사용자가 명시적으로 허용하기 전까지 제외한다.
- 단순히 계속 진행해도 되는지 확인하는 상황에서 사용자가 5분 동안 응답하지 않으면, `Scope`와 `Constraints` 안의 가장 보수적인 다음 단계를 계속한다. 침묵을 파일 삭제/이동, 설치, 다운로드, hardware 실행, scope 확장 승인으로 해석하지 않는다.

Done when:
- 다음 확장 목표 하나가 선택되어 있고 선택 이유가 현재 Knowledge Wiki 구조와 연결되어 있다.
...
Stop if:
- Knowledge Wiki index나 라우팅 파일을 읽을 수 없다.
- 필요한 경로가 실제로 존재하지 않는데 추정으로 진행해야 한다.
- 계획을 넘어 파일 이동, 삭제, 설치가 필요해진다.

Handoff:
다음 세션은 이 답변의 `Ready-to-paste next /goal`을 검토하되, 사용자가 승인한 경우에만 새 `Current Goal`로 실행한다. 중단된 경우에는 위 `First context to inspect` 파일을 먼저 읽고 첫 번째 미완료 `Done when` 항목부터 이어간다.
```
...
```text
Current Goal:
사용자가 지정한 프로젝트 코드의 가독성을 높이되 런타임 동작은 유지한다.

First context to inspect:
- TODO: path to the target repository `AGENTS.md` or equivalent project instructions
- TODO: target file path provided by the user
...
Constraints:
- 동작 변경이 필요해 보이면 먼저 사용자에게 묻는다.
- Korean comments는 사용자가 한국어 주석을 원할 때 의미가 무거운 변수나 분기에만 추가한다.
...
Stop if:
- 지정 대상 파일이 없거나 사용자의 요청 범위를 확정할 수 없다.
...
```text
Mission:
사용자가 ROS2와 Physical AI를 실제 로봇 프로젝트에 적용할 수 있도록, 작은 실습과 재사용 가능한 위키 기록을 계속 축적한다.

Current Goal:
하나의 작은 ROS2 또는 Physical AI 학습 질문을 정하고, 로컬에서 안전하게 확인 가능한 최소 실습을 수행한 뒤 practice card로 저장한다.

First context to inspect:
- TODO: path to workspace `AGENTS.md` or equivalent router instructions
- TODO: path to wiki or knowledge-base `AGENTS.md`
- TODO: path to robotics task guide, if one exists
- TODO: user-provided project, dataset, or package path

Scope:
- In scope: 한 가지 질문, 한 가지 작은 실행/확인, 한 개 practice card.
- Out of scope: 큰 simulator setup, 대용량 모델 다운로드, GPU-heavy runtime, hardware actuation.

Constraints:
- raw/source evidence와 synthesis를 분리한다.
- 공식 문서나 현재 정보가 필요한 경우에는 출처를 확인하고 링크를 남긴다.
- 비밀 정보나 private raw log를 저장하지 않는다.
- 단순 진행 확인만 필요한 경우 사용자가 5분 동안 응답하지 않으면 안전한 로컬 조사나 기록 정리처럼 `Scope` 안의 보수적인 다음 단계를 계속한다. 침묵을 대용량 다운로드, hardware 실행, 큰 simulator 실행, private data 저장 승인으로 해석하지 않는다.

Done when:
- practice card에 `목적 -> 실행 명령/코드 -> 결과 -> 실패/경고 -> 사용자가 실수했을 때 대응법 -> 다음 실습`이 채워져 있다.
...
Stop if:
- 큰 simulator 실행, hardware actuation, large download, secret/private log 저장이 필요해진다.
- 실습 환경이 없어 결과를 검증할 수 없고 추정으로만 써야 한다.

Handoff:
중단되면 practice card path와 마지막 command output을 먼저 읽고, 비어 있는 practice card section부터 이어간다. 완료된 뒤에는 사용자가 승인한 `Ready-to-paste next /goal`만 새 `Current Goal`로 실행한다.
```
